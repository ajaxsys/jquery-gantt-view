...
const gantt = function (jQuery) {
    ...
    var DateUtils = {
        ...
        daysBetween: function (start, end) {
            ...
            while (Date.compare(date,end) == -1) { count = count + 1; date.addDays(1); }
            return count;
        },
		...
		getBoundaryDatesFromData: function (data, minDays) {
			...
			for (var i = 0; i < data.length; i++) {
				for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    if (Date.compare(minStart,start) == 1) { minStart = start; }
                    if (Date.compare(maxEnd,end) == -1) { maxEnd = end; }
				}
			}
